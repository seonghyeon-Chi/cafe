...
**jsonWebToken을 활용해 토큰을 쿠키와 헤더를 통해 주고 받습니다. 미들웨어를 활용하여 토큰의 유효성 체크 역시 logout, order, payment에서 진행합니다.**
**(session과 token 인증은 함께 사용하지 않는 것이 원칙이라 알고 있지만 token 기반 인증 역시 가능하다는 것을 보이기 위해 이렇게 사용했습니다)**
<br/><br/>
